...
const RecentChat: React.FC<RecentChats> = ({allUsers,toChangeCurrentUser,onAddingNewUser, toRemoveUser}) => {

    const backgroundColorToggle = useColorModeValue(styles.lightContainer,styles.darkContainer)
    const fontColorToggle = useColorModeValue(styles.lightColor,styles.darkColor)

    const [showModal,setShowModal] = useState(false);

    const onchangingCurrentUserHandler = (name: string, number: string | null) => {
    ...
    const newUserInfo: React.FormEventHandler = (e: React.FormEvent<UsernameFormElement>) => {
        e.preventDefault();
  ...
  return (
    <Box className={`${backgroundColorToggle}`}>
        ...
        {/* Heading */}
        <Box className={`${styles.top_section} ${fontColorToggle}`}>
            Chats
        </Box>
        ...
        <Box className={styles.Chat_items_section}>
            {allUsers.map((user) => {
                return (<ChatItem key={user.number} toRemoveUser={toRemoveUser} name={user.name} phoneNumber={user.number} active={user.active} toChangeCurrentUser={onchangingCurrentUserHandler} />)
            })}
            <button onClick={toShowModal} className={styles.addUser_button}>
              <FontAwesomeIcon icon={faPlus} />
            </button>
            {showModal?<Modal onHideModal={toHideModal} onSubmitForm={newUserInfo} />:<></>}
        </Box>
    </Box>
  )
}
